...
func runProbe(cmd *cobra.Command, opts initOpts) error {
	...
	if honeycombToken == "" {
		honeycombToken = os.Getenv("HC_API_KEY") // legacy alias
	}
	...
	if honeycombToken != "" {
		...
		fmt.Fprintf(out, "\nSkipping Honeycomb (HONEYCOMB_API_KEY / HC_API_KEY not set)\n")
		result.Honeycomb = hcProbe{Skipped: true, SkipReason: "HONEYCOMB_API_KEY not set"}
	}
...
func renderProbeDraft(r probeResult) (string, error) {
	...
	fmt.Fprintf(&sb, "# generated by xray init --from-org %s --probe\n", r.Org)
