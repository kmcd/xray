...
## 027 — `internal/progress.Sink` as the cli-ux event seam; hand-rolled ANSI TTY grid; run-level emission only

**Decision.** Introduce a small `internal/progress` package containing the `Sink` interface and `Event` value type used by every `--output` mode at run time. Concrete sinks: `NopSink` (`--output quiet`), `LogSink` wrapping `*slog.Logger` (`--output log`, non-TTY `auto`), `JSONSink` emitting NDJSON (`--output json`), and `TTYSink` rendering a live `(repo × connector)` grid via hand-rolled ANSI cursor-up + clear-to-end (`--output auto` on a TTY). `cmd/xray/run.go:buildProgressSink` picks the right sink from the resolved `Mode`. `internal/run/run.go` emits `PhaseStart` / `PhaseDone` / `PhaseError` / `PhaseSkipped` at three sites: per-repo clone (inside the clone goroutine), per (repo, connector) extract (around the worker-pool `Extract` call), and postprocess. No connector internals change in this PR; connectors carry no `progress.Sink` field and emit no mid-walk `PhaseProgress` events.

**Why.** A long-running `xray run` is a wall of stderr `slog` lines today. The customer-facing requirement (#81) is a live status grid that shows which `(repo × connector)` pairs are pending, running, done, errored, or permission-skipped, plus a rough ETA. Two adjacent issues — `#82` rate-limit visibility and `#84` post-run summary — both need the same event contract to emit through, so the seam needs to land first regardless of which renderer ships beside it.

**A. Hand-rolled ANSI instead of bubbletea / tcell.** The TTY grid only needs CUU (cursor up) and ED (erase display from cursor); both are universal on the terminals xray's customers run on. The `static-binary, checksum-verifiable security-review` story (Settled context in CLAUDE.md) is the whole point of the Go choice; pulling in bubbletea + lipgloss + termenv re-raises that review on every upgrade and triggers an ADR cycle for each. Hand-rolled keeps the dependency graph unchanged.

**B. Run-level emission only this PR.** Connector paginated walks (GitHub's cursor-paginated `prListQuery` especially) have no pre-known `Total` until the walk completes, so `PhaseProgress` events from inside connectors would emit Done values without meaningful denominators. The acceptance criterion ("TTY: live grid renders, refreshes ~5 Hz, ETA shown") is satisfied with run-level start/done events alone — the grid shows `●` for running and `✔ N rows` for done; ETA derives from per-connector mean wall-clock × outstanding pairs. When `#82` lands, `RateLimit` / `Retry` events flow through the same sink (the TTY header rotates the most recent message) without any code change in the connectors. The seam is preserved for future mid-walk progress; only the call sites are deferred.

**C. Pre-registered grid via `TTYSink.Plan`.** `cmd/xray/run.go` calls `tty.Plan(repos, connectorNames, workers)` before `run.Run` so every cell starts visible as `▢ pending` rather than appearing as `PhaseStart` events arrive — gives a stable layout from the first frame, matches the issue's reference layout, and avoids the eye-jarring grow-by-rows pattern of late binding. Cells discovered later still get added.

**Out of scope.**

- *Connector-side `progress.Sink` field.* Adding it touches every connector and is unnecessary for run-level emission. Defer until a connector has a concrete need to emit mid-walk events (e.g. a long, cardinality-known REST walk where `Total` is meaningful).
- *Per-row PhaseProgress in this PR.* See B above; GraphQL cursor pagination has no `Total`.
- *Terminal-resize / mouse handling.* If a customer surfaces a real need, revisit with a separate ADR weighing bubbletea (or tcell) against the dependency cost.

**How to apply.** New: `internal/progress/{progress,nop,log,json,tty}.go` plus tests; `progress_test.go` for the contract, `log_test.go` / `json_test.go` for the byte-shape of the two non-TTY sinks (`json_test.go` asserts the wire shape documented in `docs/spec.md` "JSON event schema"), `tty_test.go` for grid render strings, ETA arithmetic, and Start/Stop lifecycle. Modified: `internal/run/options.go` adds `Progress progress.Sink`; `internal/run/run.go` emits at clone goroutine completion, worker-pool `Extract` boundary (with a small `emitExtractResult` helper translating `connector.Provenance` into the right kind), and around the postprocess call. `cmd/xray/run.go` adds `buildProgressSink` and threads the sink into `run.Options`; calls `stopSink()` before `emitRunSummary` so "wrote …" lands cleanly below the finalised grid. `README.md` "Usage" section gains a small ASCII mockup of the auto-mode grid. Closes [#81](https://github.com/kmcd/xray/issues/81).

## 028 — Graceful Ctrl-C: cooperative cancel, second-signal force-exit, exit code 130

**Decision.** `cmd/xray/main.go` replaces `signal.NotifyContext` with an explicit `signal.Notify` loop driving a two-state machine in `watchSignals`: first signal calls the run's `context.CancelFunc`, prints a one-line stderr notice, and continues listening; any subsequent signal logs `force exit; temp dir <path> not cleaned` and `os.Exit(130)`, bypassing all `defer`s. `internal/run/Run` adds an `inflightTracker` mutex-guarded map updated around every worker's `j.conn.Extract` call so the dispatcher can snapshot what was running at the moment `ctx.Done()` fired; `Result` gains `Interrupted`, `InterruptedPhase` ("clone" / "extract" / "postprocess"), `InflightJobs`, and `TempDir`. `cmd/xray/run.go` detects `errors.Is(err, context.Canceled)` and prints `run.InterruptSummary` to stderr regardless of `--output` mode before returning `silentCode(err, 130)`. The temp-dir path is wired up to the signal handler via an `Options.OnTempDir` callback that writes an `atomic.Pointer[string]` in `cmd/xray/main.go`.

**Why.** `xray run` may execute for an hour against production systems. Default Go signal handling kills the process immediately, leaks `/tmp/xray-*` clone directories, and leaves the operator without a phase-named summary. The issue (#86) classifies this as user-experience polish for v0.3.0 but the cooperative-cancel surface is also a connector invariant — `docs/spec.md` Connectors: "must support clean cancellation" — so the seam wants explicit testing rather than implicit "Go's default".

**A. Always-force on the second signal, not 5-seconds-windowed.** The issue text spells out the within-5s case and is silent on the after-5s case; a windowed implementation would surprise the operator whose second Ctrl-C "doesn't work" because they waited too long. Always-force is strictly safer: a user who Ctrl-Cs twice always wins, regardless of timing. The 5-second prose in the issue is honored as the *typical* double-tap case, not as a debounce.

**B. Inflight snapshot inside `run.Run`, not via the progress sink.** The TTY sink already tracks `(repo, connector)` state, but it tracks the *display* of state, not the live "what's executing right now" set; the dispatcher needs the latter to snapshot *before* it closes the job channel and workers begin to drain. A separate `inflightTracker` (sync.Mutex around a `map[inflightKey]struct{}`) is small, isolated to `internal/run/interrupt.go`, and has no compile-time coupling to the sink layer. The snapshot is sorted lexicographically so the rendered summary is deterministic across reruns.

**C. Temp-dir callback rather than channel or globals across the package boundary.** The signal handler in `main.go` needs the path of the run's temp directory so the force-exit log line can name what's leaking. Options considered: a channel returned from `run.Run` (lifecycle-fragile — the handler may run after Run returns), a package-level global in `internal/run` (couples `cmd` to `internal/run`'s package state), or a callback through `Options`. The callback wins on least coupling and is naturally test-isolatable: tests pass a closure that captures into a local variable; production passes a closure that writes the package-level `atomic.Pointer[string]` in `cmd/xray`. `Run` clears the snapshot before returning so a follow-up run in the same process (used by tests) can't see a stale value.

**D. Exit code 130 separate from the 0/1/2/3 contract.** The spec's exit-code table is the analyser/CLI contract for *terminations the binary chose*. 130 is the POSIX convention for "process killed by SIGINT" (128 + 2). Documenting it alongside the existing codes is the right thing for operators, but it does NOT compete with the 0/1/2/3 contract — adding 130 isn't a contract bump.

**Out of scope.**

- *Cancellation of the `archive.WriteTarGz` step.* Archive packaging starts after the worker pool has fully drained and is the last thing before Run returns; mid-archive cancel is a tiny window and the operator's recourse is the double-tap force-exit anyway. Not worth refactoring the archive API for.
- *Per-row interrupt state inside connectors.* Connectors already check `ctx.Done()` / `ctx.Err()` between pages (diff_review.md invariant). The inflight tracker records the (repo, connector) pair, which is precise enough for the operator's "what was running" question.
- *Custom exit codes for SIGTERM vs SIGINT.* SIGTERM (15) is treated identically to SIGINT — the operator hit "stop"; encoding the signal source isn't a meaningful distinction for the artifact path. Both → 130.

**How to apply.** New: `internal/run/interrupt.go` (inflightTracker + `interruptedResult` helper), `internal/run/interrupt_test.go`, `cmd/xray/main_test.go` (signal-handler state machine with injectable channel + exit func). Modified: `internal/run/options.go` adds `OnTempDir func(string)`; `internal/run/run.go` installs the tracker, calls `OnTempDir` right after `os.MkdirTemp`, snapshots on dispatcher `ctx.Done()`, adds `ctx.Err()` gates after clones and after postprocess, and populates the new `Result` fields; `internal/run/summary.go` adds `InterruptSummary` + `InterruptSummaryInput`; `cmd/xray/main.go` switches from `signal.NotifyContext` to `signal.Notify` + `watchSignals` goroutine + `atomic.Pointer[string] tmpDirRef`; `cmd/xray/run.go` passes the `OnTempDir` callback through, clears `tmpDirRef` after `Run`, detects `context.Canceled` and emits the interrupt summary on stderr; `docs/spec.md` appends a "Cancellation" subsection and adds 130 to the exit-code list; `CHANGELOG.md` records the change under `[0.3.0]`. Closes [#86](https://github.com/kmcd/xray/issues/86).
