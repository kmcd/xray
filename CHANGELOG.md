...
- **install paths: brew Cask tap (macOS), `install.sh` (Linux), Scoop bucket (Windows).** Replaces the previous 5-line `curl … | tar -xz` snippet as the primary install. New `.goreleaser.yaml` `homebrew_casks:` and `scoops:` stanzas write `Casks/xray.rb` and `bucket/xray.json` directly into this same repo on tag-release (no separate `kmcd/homebrew-xray` / `kmcd/scoop-xray` repos, no extra PAT secrets) — the release workflow's `GITHUB_TOKEN` does the commit, which requires `main` branch protection to allow the `github-actions[bot]` actor to bypass review. The Cask shape (not a Formula) follows the GoReleaser 2.10+ recommendation for pre-built binaries — `brews:` is soft-deprecated upstream. New `scripts/install.sh` is a POSIX `sh` script (~120 lines, `shellcheck -s sh` clean): detects OS + arch, resolves `releases/latest` (or honours `XRAY_VERSION`), downloads archive + `checksums.txt`, verifies sha256, optionally runs `cosign verify-blob` (`XRAY_VERIFY_COSIGN=1`), and installs to `${XRAY_INSTALL_DIR:-/usr/local/bin}/xray` without invoking `sudo` itself. New `install-sh` CI job in `ci.yml` runs `shellcheck` then exercises the script end-to-end against the latest published release on every PR. The README `## Install` section is rewritten in the order macOS → Linux → Windows → manual download → `go install`; the cosign-verify path stays linked from "Manual download" so the security-team story is preserved. ([#128])
