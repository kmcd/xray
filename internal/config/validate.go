...
func Validate(cfg *Config, meta *toml.MetaData, file string) []Diagnostic {
	...
	if c.GitHub != nil {
		validateGitHubSparseConfig(emit, c.GitHub, cfg.Window)
	}
...
// validateGitHubSparseConfig validates the sparse-historical PR sampling
// fields (pr_inflection, pr_bracket_window, pr_history_sample).
func validateGitHubSparseConfig(emit func(string, string), gh *GitHubConn, globalWindow Window) {
	...
	if hasInflection && hasBracket && globalWindow.Raw != "" {
