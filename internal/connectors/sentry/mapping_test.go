...
func TestParseNextLink(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantURL: "https://sentry.io/api/0/projects/o/p/issues/?cursor=abc",
			wantOK:  true,
		},
		{
			name:   "next with results false",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got, ok := parseNextLink(tc.header)
			if ok != tc.wantOK {
