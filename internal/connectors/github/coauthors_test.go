...
func TestTrailerCoauthors(t *testing.T) {
	...
	rows := trailerCoauthors(rec, "kmcd/foo")
	if len(rows) != 2 {
...
func TestTrailerCoauthorsNone(t *testing.T) {
	...
	if got := trailerCoauthors(rec, "r"); got != nil {
...
func TestCommitterDistinctCoauthor(t *testing.T) {
	t.Run("distinct", func(t *testing.T) {
		...
		row, ok := committerDistinctCoauthor(rec, "r")
		if !ok {
	...
	t.Run("same_identity", func(t *testing.T) {
		...
		if _, ok := committerDistinctCoauthor(rec, "r"); ok {
